...
//    var author = [];
    ...
    for (i = 0; i < quotes.length; i++) {
        ...
        splitquote[i] = quotes[i].split("@");

    }

    function randquote() {
        var quote = splitquote[Math.floor(Math.random() * quotes.length)];
        return quote;
    }

    endquote = randquote();
    tweet = endquote[0].replace(/\s/g, "%20")
        + "-%20" + endquote[1].replace(/\s/g, "%20") + "%20-";
    $("#textbox-quote").text(endquote[0]);
    $("#textbox-author").text("- " + endquote[1] + " -");
    
    var test = "https://twitter.com/intent/tweet?text="+tweet;
    $("#tweetanchor").attr("href", test);
});
